...
\begin{enumerate}
    \item $F(x)=\displaystyle \sum_{k:x_k \leq x} p(x_k)$, hvor $x\in \R$
    \item $p(x_k)=F(x_k)- \displaystyle \lim_{y \rightarrow x_k^- } F(y)$ for $k\in \N $
    \item hvis $B \subseteq \R$, så er $P(X \in B)= \displaystyle \sum_{k:x_k \in B} p(x_k)$.
\end{enumerate}
\end{prop} 
\begin{proof}
Først bevises punkt 3. Lad $V_X$ være udfaldsrummet af $X$. Vi har derved, at
\begin{align*}
